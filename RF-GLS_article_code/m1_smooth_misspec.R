...
Coeffmatrix<-expand.grid(iter = 1:100, sigma.sq = c(1,5,10), setup = c(2,3,5), percent = c(1/100,10/100,25/100))
...
setup = Coeffmatrix$setup[indica]
...
mu_2 <- c(3/4,1/2)
prob <- c(1/2, 1/2)
if(setup == 2) {height_comp <- c(1, 1/4)}
if(setup == 3) {height_comp <- c(1/4, 1/4)}
if(setup == 5) {
  height_comp <- c(1/4, 1/4)
  mu_1 <- c(1/4,1/4)
  mu_2 <- c(0.6,0.9)
}
...
set.seed(iter+100)
for(indx in 1:n){
  w_init[indx] <- dmvnorm(smat[indx,], mu_1, height_comp[1]*0.01*diag(2)) +
    dmvnorm(smat[indx,], mu_2, height_comp[2]*0.01*diag(2))
}

scaled_w <- scale(w_init)
w200 <- (sigma.sq)^0.5 * scaled_w
...
sprf_res <- spatialrandomforest_tot(x = xtrain200,y = ytrain200, coords = strain200, xtest = total_x_prediction, nsample = nrow(xtrain200), nthsize = nthsize,
                                    mdim = ncol(xtrain200), ntree = ntree, ntest = nrow(total_x_prediction), mtry = max(1,floor(ncol(xtrain200)/3)),sigmasq = est_theta$Theta[1], tausq = est_theta$Theta[2], phi = est_theta$Theta[3], ytest = NULL, fxin = fxtrain200, fxout = c(fxtest200, fxestimation200), trim = 0.00)
